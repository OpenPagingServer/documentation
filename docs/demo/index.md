...
The server is setup to replicate a school-district. With multiple campuses/buildings. Each group is named after a fictional building in this district. Names of groups are humorous and each either make reference to pop culture, an inside joke from staff members, or a lighthearted joke. Each one has different types of endpoints. You can use Multicast Gateway to receive multicast RTP for both generic streams and Polycom streams. Each also has it's own sets of bells.
...
| The Royal Cup of Tea Private Wear Your Uniform!! Secondary School | Generic Multicast RTP Stream. For use with Cisco SPA/MPP, Yealink, Grandstream, Snom, etc. Devices must be configured for PMCA. | 232.9.10.11:23456 PMCA                           |
